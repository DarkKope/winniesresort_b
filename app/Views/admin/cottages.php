...
<div class="main-content">
    <div class="d-flex justify-content-between align-items-center mb-4"><h2>Manage Cottages</h2><a href="/admin/add-cottage" class="btn btn-primary"><i class="fas fa-plus"></i> Add Cottage</a></div>
    <div class="card"><div class="card-body p-0"><div class="table-responsive"><table class="table mb-0"><thead><tr><th>ID</th><th>Cottage Name</th><th>Price/Day</th><th>Capacity</th><th>Status</th><th>Actions</th></tr></thead><tbody>
    <?php foreach($cottages as $cottage): ?>
    <tr><td><?= $cottage['cottage_id'] ?></td><td><strong><?= $cottage['cottage_name'] ?></strong></td><td>₱<?= number_format($cottage['price_per_day'], 2) ?></td><td><?= $cottage['capacity'] ?></td><td><span class="badge bg-<?= $cottage['status'] == 'available' ? 'success' : 'danger' ?>"><?= ucfirst($cottage['status']) ?></span></td><td><a href="/admin/edit-cottage/<?= $cottage['cottage_id'] ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a> <a href="/admin/delete-cottage/<?= $cottage['cottage_id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Delete?')"><i class="fas fa-trash"></i></a></td></tr>
    <?php endforeach; ?>
